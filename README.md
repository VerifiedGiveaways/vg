...
The Verified Giveaways project uses II authentication. When running the dApp locally, you may authenticate with a local instance of the II provider or fake the provider with a config setting. (Note: Mainnet Internet Identities only work with the mainnet canister. They will not work when running locally in an emulated environment.)

You can fake the II provider with an autogenerated identity and skip the remaining setup for step 2. Simply add the following config setting to your .env.local file:
```bash
